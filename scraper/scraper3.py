...
class Immoweb_Scraper:
    ...
    def process_url(self, each_url):
        ...
        try:
            for tag in soup.find("div",attrs={"id" : "classified-description-content-text"}).find_all("p"):
                if any(keyword in tag.text.lower() for keyword in ["open haard", "cheminee", "feu ouvert", "open fire"]):
    ...
    def Clean_DataFrame(self):
        ...
        col_to_conv = ['Price', 'Swimming pool', 'Construction year','Number of frontages', 'Living area', 'Bedrooms', 'Terrace surface', 'Surface of the plot', 'Garden surface','Open Fire'] 
