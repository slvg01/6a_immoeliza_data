...
class Immoweb_Scraper:
    ...
    def Clean_DataFrame(self):
        ...
        self.data_set_df["Building conditon status"] = self.data_set_df[
            "Building condition"
        ].apply(
            lambda x: "None"
            if x == "None"
        ...
        self.data_set_df["Furnished"] = self.data_set_df["Furnished"].apply(
            lambda x: "None" if x == "None" else (1 if x == "yes" else 0)
        )
        self.data_set_df["Kitchen equipped"] = self.data_set_df["Kitchen type"].apply(
            lambda x: "None"
            if x == "None"
            else (0 if x in ["Notinstalled", "USAuninstalled"] else 1)
        )
        self.data_set_df["Terrace"] = self.data_set_df["Terrace surface"].apply(
